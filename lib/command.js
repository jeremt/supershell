 ...
 */
Command.prototype.run = function (cmd, args) {
  var _this = this;
  var output = '';
  ...
  if (this._cmd === undefined) this._cmd = cmd;
  if (this._args === undefined) this._args = args || [];

  if (config.alias[cmd] !== undefined) {
    this._cmd = config.alias[cmd].cmd;
    this._args = this._args.concat(config.alias[cmd].args);
  }

  this._result = exec(this.toString());
  // save a timestamp of when the command started.
  var timestamp = new Date().toString();
  this._result.stdout.on('data', function (data) {
    output += data;
  });
  this._result.stderr.on('data', function (data) {
    output += data;
  });
  this._result.on('close', function (code) {
    _this.state = 'done';
    var finalOutput = output;
    for (var i = 0; i < _this._pipeFunctions.length; ++i)
      finalOutput = _this._pipeFunctions[i](finalOutput, code);
    _this.emit('finish', finalOutput, _this.cmd);
    if (_this.previous === null)
      config.history.unshift({timestamp: timestamp, command: _this.toJson()});
    if (code) {
      _this.emit('fail', finalOutput, _this.cmd, code);
      if (_this._orCommand)
        return _this._orCommand.run();
      else if (_this._thenCommand)
        return _this._thenCommand.run();
    }
    else {
      _this.emit('success', finalOutput, _this.cmd);
      if (_this._andCommand)
        return _this._andCommand.run();
      else if (_this._thenCommand)
        return _this._thenCommand.run();
    }
  });
...
Command.prototype.pipe = function (cmd, args) {
  if (cmd.constructor.name === 'Function') {
    if (this.state === 'done')
      console.log('Already done so your pipe won\'t be applied.')
    this._pipeFunctions.push(cmd);
  }
  else {
    if (this.state === 'running' || this.state === 'done')
      console.warn('Already running so your pipe won\'t be applied.');
...
exports.run = function (cmd, args, kw) {
  return new Command().run(cmd, args, kw);
}

...
/**
 * Checkout a file from its name.
 */
git.command('checkout', function (e, name, type) {
  type = type || "branch";
  _exec(e, ['checkout', type === "branch" ? '' : '--', name]).pipe(sh.parsers.trim());
}, [_isInit]);
...
git.command('push', function (e, force) {
  _exec(e, [
    'push', force ? '--force' : '',
    git.config.remote, git.config.branches.current
  ]);
}, [_isInit, _isConnected, _isNotRebasing]);

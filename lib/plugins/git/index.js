...
git.command('branch', function (e, name) {
  _exec(e, ['checkout', '-b', name], function () {
    _exec(e, ['push', '-u', git.scope.remote, name]);
  }, []);
});
...
git.command('removeBranch', function (e, name) {
  _exec(e, ['branch', '-D', name], function () {
    _exec(e, ['push', git.scope.remote, ":" + name]);
  });
});
...
/**
 * Merge changes from the given branch.
 */
git.command('merge', function (e, branch) {
  _exec(e, ['merge', branch]);
}, [_isInit, _isConnected, _isNotRebasing]);

/**
 * Pull changes from current branch's remote.
 */
git.command('pull', function (e, rebase) {
  _exec(e, [
    'pull', rebase ? '--rebase' : '',

    ...
    if (!exists)
      ...
      fs.exists(path.join(folder, '.git'), function (exists) {
        if (!exists)
          ...
          git.scope.folder = folder;
          sh('git config remote.' + git.scope.remote + '.url')
            .pipe(sh.parsers.trim())
            .on('success', function (output) {
...
git.command('resolve', function (e) {
  dns.resolve(git.scope.url, function (err) {
 ...
 */
git.command('file', function (e, file) {
 ...
 */
git.command('checkout', function (e, name) {
 ...
 */
git.command('rebase', function (e, option) {
  if (git.scope.rebasing === true) {
    if (['skip', 'abort', 'continue'].indexOf(option) === -1)
      e.emit('fail', 'unknown rebase option --' + option);
    else
      _exec(e, ['rebase', '--' + option]);
  }
  else {
    _exec(e, [
      'pull', '--rebase', git.config.remote, git.config.branches.current
    ]);
  }
 ...
 */
git.command('push', function (e) {
  _exec(e, ['push', git.config.remote, git.config.branches.current]);
}, [_isInit, _isConnected, _isNotRebasing]);

/**
 * Sync target branch with the given source using rebase command.
 */
